...
export default function ShiftCreateForm({ staff }: { staff: StaffUser[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Rol del turno</label>
        <input
          type="text"
          required
          placeholder="Ej: Recepción, Limpieza, Cocina"
          value={role}
      ...
      >
        {status === 'loading' ? 'Creando...' : 'Crear turno y notificar'}
      </button>
    </form>
  )
}
